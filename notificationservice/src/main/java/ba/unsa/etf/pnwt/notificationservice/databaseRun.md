...
```bash
docker run --name notification-mysql -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE=notification_db -e MYSQL_USER=notif_user -e MYSQL_PASSWORD=notif_pass -p 3312:3306 -d mysql:8
```
